...
FILE *infile;
FILE *outfile;
char *outfilename = "errorout12.txt";
// Lexicographical level
int current_level = -1;

// Functions
int getElfFormat(char* opcode);
...
void term();
void factor();
void error(int errNum, char* str, int newln);
...
int main(int argc, char* argv[]) {
  ...
  if(argc < 2) {
    ...
    if(scanf("%s", infilename) != 1) {
      error(0, "could not open input file", 0);
    }
  }
  ...
  if(!infile)
    error(0, "file is empty or does not exist", 0);
      ...
      default:
        ...
        if(isalpha(formattedInput[i])) {
          while(isalpha(formattedInput[i]) || isdigit(formattedInput[i])) {
            ...
            if(tempStrLength > MAX_IDENTIFIER_LENGTH) {
              // Output exit message
              error(25, "identifier too long", 1);
            }
            i++;
          }
        ...
        // Integer case
          ...
          if (isalpha(formattedInput[i])) {
            ...
            error(27, "identifier starts with digit", 1);
          }
        ...
        // Output exit message
          error(28, "invalid symbol", 1);
        }
    }
  }

  // Parsing - main program
  // Add main as first entry to symbol table
  ...
  // Output message that no errors occurred
  ...
  // Output to elf.txt file
  outfile = fopen("elf.txt", "w");
  for(int i=0; i<curr_assembly_code_table_size; i++) {
    int tempCode = getElfFormat(assembly_code_table[i].opcode);
    fprintf(outfile, "%d %d %d\n", tempCode, assembly_code_table[i].l, assembly_code_table[i].m);
}
...
int getElfFormat(char* opcode) {
  ...
  if(strcmp(opcode,"LIT") == 0)
    tempCode = 1;
  else if(strcmp(opcode,"OPR") == 0)
    tempCode = 2;
  else if(strcmp(opcode,"LOD") == 0)
    tempCode = 3;
  else if(strcmp(opcode,"STO") == 0)
    tempCode = 4;
  else if(strcmp(opcode,"CAL") == 0)
    tempCode = 5;
  else if(strcmp(opcode,"INC") == 0)
    tempCode = 6;
  else if(strcmp(opcode,"JMP") == 0)
    tempCode = 7;
  else if(strcmp(opcode,"JPC") == 0)
    tempCode = 8;
  else if(strcmp(opcode,"SYS") == 0)
    tempCode = 9;
  else //default to opr: add, mult, etc.
    tempCode = 2;
...
void block() {
  ...
  while(tokenizedInput[curr_token_idx] == procsym) {
    proc_declaration();
  }
  // Update assembly code 
  assembly_code_table[current_level].m = (curr_assembly_code_table_size * 3);
  //symbol_table[curr_symbol_table_size - 1].addr = curr_assembly_code_table_size;
  emit("INC", 0, space);
  statement();
  if(current_level > 0)
    emit("RTN", 0, 0);
...
void const_declaration() {
  do {
    ...
    if(tokenizedInput[curr_token_idx] != identsym) {
      error(2, "missing const identifier", 1);
    }
    if(symbolTableCheck(lexmeIdentifiers[curr_token_idx].identifiers) != -1) {
      error(3, "const identifier already declared", 1);
    }
    ...
    if(tokenizedInput[curr_token_idx] != eqlsym) {
      error(4, "missing const initial value", 1);
    }
    curr_token_idx++;
    if(tokenizedInput[curr_token_idx] != numbersym) {
      error(5, "const value must be a number", 1);
    }
    symbol_table[curr_symbol_table_size].val = atoi(lexmeIdentifiers[curr_token_idx].identifiers);
    symbol_table[curr_symbol_table_size].level = current_level;
    symbol_table[curr_symbol_table_size].addr = 0;
    symbol_table[curr_symbol_table_size].mark = 1;
    curr_symbol_table_size++;
    curr_token_idx++;
  ...
  if(tokenizedInput[curr_token_idx] != semicolonsym) {
    error(6, "missing semicolon after const declaration", 1);
  }
  curr_token_idx++;
}
...
int var_declaration() {
  ...
  if(tokenizedInput[curr_token_idx] == varsym) {
    do {
      curr_token_idx++;
      if (tokenizedInput[curr_token_idx] != identsym) {
        error(16, "missing var identifier", 1);
      }
      if (symbolTableCheck(lexmeIdentifiers[curr_token_idx].identifiers) != -1) {
        error(17, "var identifier already declared", 1);
      }
      ...
      symbol_table[curr_symbol_table_size].level = current_level;
      symbol_table[curr_symbol_table_size].addr = space;
      curr_symbol_table_size++;
      curr_token_idx++;
      space++;
    } while (tokenizedInput[curr_token_idx] == commasym);
    if (tokenizedInput[curr_token_idx] != semicolonsym) {
      error(18, "missing semicolon after var declaration", 1);
    }
    curr_token_idx++;
  }
  return space;
...
void proc_declaration() {
  curr_token_idx++;
  if(tokenizedInput[curr_token_idx] != identsym)
    error(19, "missing proc identifier", 1);
  if(symbolTableCheck(lexmeIdentifiers[curr_token_idx].identifiers) != -1)
    error(20, "proc identifier already declared", 1);
  // Add proc ident to symbol table
  symbol_table[curr_symbol_table_size].kind = 3;
  strcpy(symbol_table[curr_symbol_table_size].name, lexmeIdentifiers[curr_token_idx].identifiers);
  symbol_table[curr_symbol_table_size].val = 0; //intitialize vars to 0...
  symbol_table[curr_symbol_table_size].level = current_level;
  symbol_table[curr_symbol_table_size].addr = curr_assembly_code_table_size;
  curr_symbol_table_size++;
  curr_token_idx++;
  if(tokenizedInput[curr_token_idx] != semicolonsym)
    error(21, "missing semicolon after proc declaration", 1);
  curr_token_idx++;
  block();
  if(tokenizedInput[curr_token_idx] != semicolonsym)
    error(12, "missing semicolon after end of proc declaration", 1);
  curr_token_idx++;
}
...
void statement() {
  if(tokenizedInput[curr_token_idx] == identsym) {
    ...
    if(symIdx == -1) {
      error(7, "identifier not found in symbol table", 1);
    }
    if(symbol_table[symIdx].kind != 2) {
      error(8, "you can only change the value of vars", 1);
    }
    curr_token_idx++;
    if(tokenizedInput[curr_token_idx] != becomessym) {
      error(9, "missing becomes symbol", 1);
    }
    curr_token_idx++;
  ...
  else if(tokenizedInput[curr_token_idx] == callsym) {
    curr_token_idx++;
    if(tokenizedInput[curr_token_idx] != identsym)
        error(22, "missing call identifier", 1);
    if(symbolTableCheck(lexmeIdentifiers[curr_token_idx].identifiers) == -1) {
      error(7, "identifier not found in symbol table", 1);
    }
    int symIdx = symbolTableCheck(lexmeIdentifiers[curr_token_idx].identifiers);
    emit("CAL", current_level - symbol_table[symIdx].level, (symbol_table[symIdx].addr * 3));
    curr_token_idx++;
  }
  else if(tokenizedInput[curr_token_idx] == beginsym) {
    do {
      curr_token_idx++;
      statement();
    } while (tokenizedInput[curr_token_idx] == semicolonsym);
    if (tokenizedInput[curr_token_idx] != endsym) {
      error(10, "missing end symbol", 1);
    }
    curr_token_idx++;
    //return;
  }
  if(tokenizedInput[curr_token_idx] == ifsym) {
    curr_token_idx++;
    condition();
    int jpcIdx = curr_assembly_code_table_size;
    //int jpcIdx = curr_token_idx;
    emit("JPC", 0, 0);
    if(tokenizedInput[curr_token_idx] != thensym) {
      error(11, "missing then symbol", 1);
    }
    curr_token_idx++;
    statement();
    assembly_code_table[jpcIdx].m = curr_assembly_code_table_size * 3;
  ...
  if(tokenizedInput[curr_token_idx] == whilesym) {
    ...
    if (tokenizedInput[curr_token_idx] != dosym) {
      error(23, "missing do symbol", 1);
    }
    //curr_token_idx++;
    //int jpcIdx = curr_token_idx;
    int jpcIdx = curr_assembly_code_table_size;
    emit("JPC", current_level, 0);
    curr_token_idx++;
    statement();
    emit("JMP", current_level, loopIdx * 3);
    //assembly_code_table[jpcIdx].m = curr_token_idx;
    assembly_code_table[jpcIdx].m = curr_assembly_code_table_size;
    //return;
  }
  else if(tokenizedInput[curr_token_idx] == readsym) {
    curr_token_idx++;
    if(tokenizedInput[curr_token_idx] != identsym) {
      error(24, "missing ident symbol", 1);
    }
    int symIdx = symbolTableCheck(lexmeIdentifiers[curr_token_idx].identifiers);
    if(symIdx == -1) {
      error(7, "identifier not found in symbol table", 1);
  ...
  else if(tokenizedInput[curr_token_idx] == writesym) {
    curr_token_idx++;
    expression();
    emit("SYS", 0, 1);
    //return;
...
void condition() {
  if(tokenizedInput[curr_token_idx] == oddsym) {
    ...
    if(tokenizedInput[curr_token_idx] == eqlsym) {
      ...
      error(13, "condition must contain comparison operator", 1);
    }
  }
}
...
void factor() {
  if(tokenizedInput[curr_token_idx] == identsym) {
    ...
    if(tokenizedInput[curr_token_idx] != rparentsym) {
      error(14, "missing right parenthesis", 1);
    }
    curr_token_idx++;
  }
  else {
    error(15, "input needs to be ident, num, or appropriate parenthesis", 1);
  }
}
...
void error(int errNum, char* str, int newln) {
  ...
  printf("Error: Error number %d: %s", errNum, str);
      fprintf(outfile, "Error: Error number %d: %s", errNum, str);
